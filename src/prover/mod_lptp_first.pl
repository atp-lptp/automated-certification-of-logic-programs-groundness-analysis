...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% lptp_check(+F,+Pr): check if Pr is a LPTP proof of F
lptp_check(F,P) :-
  gensym(l, Id),
  (lemma(Id, F, P) -> (print_green('% LPTP: ok'),nl) ; (print_red('% LPTP: no'),nl)).

print_red(Text) :- format('\e[31m~w\e[0m', [Text]). % from ChatGPT!
    %write(Text).
print_green(Text) :- format('\e[32m~w\e[0m', [Text]).
    %write(Text).



% deduction_thm(+Gamma,-F,+Concl,+Pr,-Assumes),
deduction_thm([],Concl,Concl,Pr,Pr).
deduction_thm([H],H => Concl,Concl,Pr,assume(H,Pr,Concl)) :- !.
deduction_thm([H1,H2|Hs], H1 => Gs, Concl, Pr, assume(H1,assume(H2,Proof,Concl2),H2 => Concl2)) :-
    deduction_thm([H2|Hs],Gs,Concl,Pr,assume(H2,Proof,Concl2)).

% lptp_check(+Gamma,+IdCase,+Formula,+Proof):
%       checks if  Proof is an LPTP proof of Formula assuming Gamma
lptp_check(Gamma,IdCase,Formula,Proof) :-
  gensym(l, Id),
  deduction_thm(Gamma,Hyps_Formula,Formula,Proof,Assumes),
  %writeln(Hyps_Formula),
  ( lemma(Id, Hyps_Formula, Assumes)
    %lptp_check_on_file(Id,Hyps_Formula,Assumes)
  ->
    print_green('% LPTP: yes : '),write(IdCase),write(' - ')
  ;
    print_red('% LPTP: no : '),write(IdCase),write(' - '),
    %writeln(IdCase),
    %writeln(Gamma), writeln(Formula),
    %writeln(Proof), writeln(Assumes),
    throw('Incorrect LPTP proof'),
    true).


lptp_check_on_file(Id,Formula,Proof) :-
    F = 'test',
    ProofFile = 'test.pr',
    open(ProofFile,write,Stream),
    write(Stream,':- initialize.'), nl(Stream),
    write(Stream,':- '), write(Stream,lemma(Id,Formula,Proof)), writeln(Stream,'.'),
    write(Stream,':- bye.'), nl(Stream), close(Stream),
    atom_concat('cat ',ProofFile,Cat),
    shell(Cat),
    atom_concat('~/Desktop/lptp/bin/lptp <<< \'check(',F,Cmd0),
    atom_concat(Cmd0,'),halt.\'',Cmd),
    shell(Cmd),
    write('% '),write(Formula),write(': proof checked'),nl.
...
io__lptp_home('/Users/fred 1/Desktop/lptp').
